...
public class SuggestionController {
    ...
    public ResponseEntity<?> getAllSuggestions(
        ...
        if (paginated) {
            ...
            Page<SuggestionResponse> dtoPage = suggestions.map(SuggestionResponse::new);
            return ResponseEntity.ok(dtoPage);
        } else {
            List<Suggestion> suggestions = suggestionService.getAllSuggestions();

            List<SuggestionResponse> dtoList = suggestions.stream()
    ...
    public ResponseEntity<?> submitSuggestion(@RequestBody SuggestionRequest request,
        ...
        Long employeeId = userDetails.getId(); // from your UserDetailsImpl
        suggestionService.createSuggestion(request.getTitle(),request.getDescription(),employeeId,request.isAnonymous());
     ...
     */
    @GetMapping("/employee")
    public ResponseEntity<List<Suggestion>> getSuggestionsByEmployee(  @AuthenticationPrincipal UserDetailsImpl userDetails) {
        try {
            // 🔹 Assuming your UserDetails contains the employeeId
